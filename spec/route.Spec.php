...
describe(
    'Route test', function () {
        ...
        describe(
            'Creation', function () {
                it(
                    'should create an route with action/controler action', function () {
                ...
                it(
                    'should create an route with callback action', function () {
                        $new_route = new Route(
                            'name_path', 'uri', function () {
                            }
                        );
                        expect($new_route)->toBeA('object');
                        expect($new_route)->toBeAnInstanceOf(\fzed51\Core\Route::class);
        ...
        describe(
            'Path Analyse', function () {
                it(
                    'should interpret a simple path', function () {
                        $new_route = new Route('name_path', 'uri', 'action@controleur');
                        expect($new_route->pathToRegEx())->toBe('`^uri$`');
                    }
                );
                it(
                    'should interpret a simple path with simple parameter', function () {
                        $new_route = new Route('name_path', 'uri/{param}', 'action@controleur');
                        expect($new_route->pathToRegEx())->toBe('`^uri/(?<param>[A-Za-z0-9._-]+)$`');
                    }
                );
                it(
                    'should interpret a simple path with complex parameter', function () {
                ...
                it(
                    'should interpret a complex path with complex parameter', function () {
                ...
                it(
                    'should interpret a simple path with complex parameter with capturing group', function () {
        ...
        describe(
            'Execution', function () {
                ...
                it(
                    'should execut an route with action/controler action', function () {
                        class foo
            {
                            public function bar()
                            {
                        ...
                        $closure = function () use ($new_route) {
                ...
                it(
                    'should execut an route with callback action', function () {
                        $callback = function () {
                            echo "callback ok";
                        };
